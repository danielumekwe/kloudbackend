@extends('layouts.app')
@section('title', 'Ticket #' . ($ticket['tid'] ?? $ticket['ticketid'] ?? ''))
@section('breadcrumb')
    <a href="{{ route('support.index') }}" class="hover:text-slate-700 dark:hover:text-slate-200">Support</a>
    <span class="mx-2">/</span>
    <span class="text-slate-700 dark:text-slate-200">Ticket #{{ $ticket['tid'] ?? $ticket['ticketid'] ?? '' }}</span>
@endsection
...
<div class="flex items-start justify-between mb-6 flex-wrap gap-4">
    <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">
            Ticket #{{ $ticket['tid'] ?? $ticket['ticketid'] ?? '' }}
