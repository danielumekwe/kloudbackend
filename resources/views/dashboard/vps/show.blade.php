    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card">
                ...
                <div x-show="message" class="mb-4 text-sm p-3 rounded-lg" :class="success ? 'bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400' : 'bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400'" x-text="message"></div>
            ...
            <div class="card">
                ...
                <p class="text-sm text-red-600 dark:text-red-400 mb-4">Warning: this destroys all data on the VPS. There is no rollback.</p>
                <div class="space-y-3">
                    <div>
            ...
            <div class="card border-red-200 dark:border-red-500/20">
                <h3 class="font-semibold text-slate-900 dark:text-white mb-2">Cancel VPS</h3>
                <p class="text-sm text-red-600 dark:text-red-400 mb-4">Warning: this permanently cancels and deletes this VPS, including all data and backups. There is no rollback, and billing for this VPS stops once cancelled.</p>
            ...
            <div class="card">
                ...
                <div x-show="advancedOpen" class="mt-5 space-y-6">
                    ...
                    <div>
                        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Network</h4>
                        <div class="flex flex-wrap gap-3 items-end mb-3">
                            <div class="flex-1 min-w-[200px]">
                                <label class="form-label">Hostname</label>
                                <input type="text" x-model="hostname" class="form-input" placeholder="server.example.com">
                            </div>
                            <button @click="run('changehostname', { hostname: hostname })" :disabled="busy || !hostname" class="btn btn-primary">Update Hostname</button>
                        </div>
                        <button @click="run('buyip')" :disabled="busy" class="btn btn-secondary">Buy Additional IP</button>
                    </div>

                    <div>
                        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Reverse DNS</h4>
                        <div class="flex flex-wrap gap-3 items-end">
                            ...
                            <button @click="run('reversedns', { ips: { [rdnsIp]: rdnsHostname } })" :disabled="busy || !rdnsIp || !rdnsHostname" class="btn btn-primary">Update PTR Record</button>
                        </div>
                    </div>

                    <div>
                        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Storage</h4>
                        <div class="flex flex-wrap gap-3 items-end mb-3">
                            <div class="flex-1 min-w-[160px]">
                                <label class="form-label">Additional disk (GB)</label>
                                <input type="number" min="1" max="100" x-model.number="hdSize" class="form-input">
                            </div>
                            <button @click="run('buyhdspace', { size: hdSize })" :disabled="busy || !hdSize" class="btn btn-primary">Buy / Resize Disk</button>
                        </div>
                        <div class="flex flex-wrap gap-3 items-end">
                            <div class="flex-1 min-w-[160px]">
                                <label class="form-label">Slice count</label>
                                <input type="number" min="1" max="32" x-model.number="sliceCount" class="form-input">
                            </div>
                            <button @click="run('resizeslices', { slices: sliceCount })" :disabled="busy || !sliceCount" class="btn btn-primary">Resize Slices</button>
                        </div>
                    </div>
        ...
        <div class="card h-fit">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between"><span class="text-slate-400">Hostname</span><span class="font-medium text-slate-900 dark:text-white">{{ $live['vps_hostname'] ?? '—' }}</span></div>
                <div class="flex justify-between"><span class="text-slate-400">IP Address</span><span class="font-medium text-slate-900 dark:text-white">{{ $live['vps_ip'] ?? '—' }}</span></div>
                <div class="flex justify-between"><span class="text-slate-400">Plan</span><span class="font-medium text-slate-900 dark:text-white">{{ $live['services_name'] ?? '—' }}</span></div>
                <div class="flex justify-between"><span class="text-slate-400">Slices</span><span class="font-medium text-slate-900 dark:text-white">{{ $order->config['slices'] ?? '—' }}</span></div>
