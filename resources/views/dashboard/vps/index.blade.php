...
@else
    @php
        $rows = $instances->map(function ($item) {
            $order = $item['order'];
            $live  = $item['live'];
            return [
                'order' => $order,
    ...
    {{-- Mobile: stacked cards (the 9-column table below is unreadable under ~640px) --}}
    <div class="space-y-3 sm:hidden">
        @foreach($rows as $row)
        @php $order = $row['order']; $live = $row['live']; @endphp
        <a href="{{ route('vps.show', $order->id) }}" class="card block">
            <div class="flex items-start justify-between gap-3 mb-3">
                <div class="min-w-0">
                    <p class="font-medium text-slate-900 dark:text-white truncate">{{ $order->config['hostname'] ?? 'VPS #' . $order->id }}</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">#{{ $order->id }} &middot; {{ config('vps_pricing.categories.' . $order->category . '.label', $order->category) }}</p>
                </div>
                <span class="badge {{ $row['badgeClass'] }} flex-shrink-0">{{ $row['statusLabel'] }}</span>
            </div>
            <div class="grid grid-cols-2 gap-y-1.5 text-xs">
                <span class="text-slate-400">IP Address</span>
                <span class="text-slate-700 dark:text-slate-300 text-right">{{ $live['vps_ip'] ?? '—' }}</span>
                <span class="text-slate-400">Slices</span>
                <span class="text-slate-700 dark:text-slate-300 text-right">{{ $order->config['slices'] ?? '—' }}</span>
                <span class="text-slate-400">Price</span>
                <span class="text-slate-700 dark:text-slate-300 text-right">${{ number_format($order->price, 2) }}</span>
                <span class="text-slate-400">Ordered</span>
                <span class="text-slate-700 dark:text-slate-300 text-right">{{ $order->created_at->format('M j, Y') }}</span>
            </div>
        </a>
        @endforeach
    </div>
