@extends('layouts.app')
@section('title', 'My Quick Servers')
@section('breadcrumb', 'My Quick Servers')

@section('content')
<div class="flex items-center justify-between mb-6 flex-wrap gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">My Quick Servers</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Your rapid-deploy dedicated servers, powered by InterServer</p>
    </div>
</div>
...
@if($instances->isEmpty())
    <div class="card text-center py-16">
        ...
        <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-1">No Quick Servers yet</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Pick a Quick Server plan from the sidebar to get started.</p>
    </div>
@else
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        @php
            ...
            $statusLabel = match($order->status) {
                'provisioned'     => $live['qs_status'] ?? 'Active',
                'paid'            => 'Provisioning…',
        ...
        <a href="{{ route('qs.show', $order->id) }}"
           class="card hover:shadow-md hover:border-blue-500/30 dark:hover:border-blue-500/30 transition-all duration-200 group">
            ...
            <h3 class="font-semibold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-0.5">
                {{ $live['qs_hostname'] ?? 'Quick Server #' . $order->id }}
            </h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Quick Server</p>

            <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                <div>
                    <span class="text-slate-400 dark:text-slate-500">IP Address</span>
                    <p class="font-medium text-slate-700 dark:text-slate-300 mt-0.5">{{ $live['qs_ip'] ?? '—' }}</p>
                </div>
                <div>
                    <span class="text-slate-400 dark:text-slate-500">OS</span>
                    <p class="font-medium text-slate-700 dark:text-slate-300 mt-0.5">{{ $order->config['os'] ?? '—' }}</p>
                </div>
                <div>
            ...
            <div class="mt-4 pt-4 border-t border-slate-100 dark:border-white/[0.05] flex items-center justify-between">
                <span class="text-xs text-blue-600 dark:text-blue-400 font-medium group-hover:underline">Manage Quick Server →</span>
            </div>
        </a>
        @endforeach
    </div>
